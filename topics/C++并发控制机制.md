...
解决思路有两类：
1. **锁（mutex）**——让临界区"互斥"，同一时刻只有一个线程进入。
2. **原子变量（atomic）**——让单个操作"不可分割"，无需加锁。
...
最底层、保证一定 lock-free 的原子布尔，常用于实现**自旋锁**：
...
> 注：现代 x86/ARM 都有**缓存一致性协议（MESI）**保证缓存最终一致，但"最终"和"立即"之间的时间窗口，加上下面的乱序，才是真正的麻烦。
...
结果：在别的核心看来，`load y` 仿佛发生在 `store x` 之前——**写-读被重排了**。这正是著名的 **StoreLoad 重排**，也是 x86 唯一允许的重排。
...
`compare_exchange` 是几乎所有无锁算法的核心，必须吃透。
...
如果操作 A *happens-before* 操作 B，那么 A 的所有效果对 B **可见**。内存序的本质，就是**建立跨线程的 happens-before 关系**。
